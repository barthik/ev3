 ...
 * The digital EV3 Infrared Seeking Sensor detects proximity to the robot and
 * reads signals emitted by the EV3 Infrared Beacon. The sensor can alse be used
 * as a receiver for a Lego Ev3 IR remote control.
 *
 *
 * <table border=1>
 * <tr>
 ...
 * <b>EV3 Infra Red sensor</b><br>
 *
 * The sensor can be used as a receiver for up to four Lego Ev3 IR remote
 * controls using the {@link #getRemoteCommand} and {@link #getRemoteCommands}
 * methods.
 *
 *
 *
 ...
 *
 */
public class IRSensor extends UARTSensor {
    ...
    /**
     * Return the current remote command from the specified channel. Remote
     * commands are a single numeric value which represents which button on the
     * Lego IR remote is currently pressed (0 means no buttons pressed). Four
     * channels are supported (0-3) which correspond to 1-4 on the remote. The
     * button values are:<br>
     * 1 TOP-LEFT<br>
     * 2 BOTTOM-LEFT<br>
     * 3 TOP-RIGHT<br>
     * 4 BOTTOM-RIGHT<br>
     ...
     * The sample contains one element giving the distance to an object in front
     * of the sensor. The distance provided is very roughly equivalent to meters
     * but needs conversion to give better distance. See product page for
     * details. <br>
     ...
     * In seek mode the sensor locates up to four beacons and provides bearing
     * and distance of each beacon.
     *
     * <p>
     * <b>Size and content of the sample</b><br>
     * The sample contains four pairs of elements in a single sample. Each pair
     * gives bearing of and distance to the beacon. The first pair of elements
     * is associated with a beacon transmitting on channel 0, the second pair
     * with a beacon transmitting on channel 1 etc.<br>
     * The bearing values range from -25 to +25 (with values increasing
     * clockwise when looking from behind the sensor). A bearing of 0 indicates
     * the beacon is directly in front of the sensor. <br>
     * Distance values are not to scale. Al increasing values indicate
